...
def encrypt_field(value):
    if value is None:
        return None
    return fernet.encrypt(value.encode()).decode()  # store as string in DB
...
def decrypt_field(field_value):
    ...
    try:
        ...
        # Leave encrypted-looking values; do NOT return them unchanged
        return field_value
...
# -------------------- Fernet Key (use the one you gave) --------------------
...
def verify_password(input_password: str, stored_password: str) -> bool:
    ...
    if stored_password is None:
        return False
    try:
        # detect hashed storage (hex length 64)
...
def anonymize_all_unanonymized():
    ...
    for p in patients:
        ...
        name = name or ""
        contact = contact or ""
        # create masked fields
        anon_name = f"ANON_{pid + 1000}"
        anon_contact = f"XXX-XXX-{contact[-4:]}" if contact else "XXX-XXX-XXXX"
        # encrypt originals (if not already encrypted)
       # encrypt originals (ONLY if they are not already encrypted)
    ...
    conn.commit()
    conn.close()
    return len(patients)  # number of records updated


# -------------------- Patient CRUD --------------------
def get_all_patients_raw():  # admin raw view (decrypted)
    conn = sqlite3.connect(DB_PATH)
...
def get_patients_for_doctor():  # anonymized view
    conn = sqlite3.connect(DB_PATH)
...
def update_patient_admin(patient_id, name=None, contact=None, diagnosis=None):
    ...
    # Encrypt only if new value provided AND not already encrypted
...
# Export decrypted columns for admin
    if not df.empty:
